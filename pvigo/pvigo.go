...
func (cmd *Worker) Run() {
	...
	for _, c := range cmd.Message_Params.Params.Commands {
		...
		if err != nil {
			log.Error().Msgf("Command: %v execution error: %s\n  HATAMESAJ: %s", c.Cmd, err.Error(), string(out))
			cmd.Message_Params.Status.Set("FAILURE")
			cmd.Output <- cmd.Message_Params
			return
		}
	...
	files, err := filepath.Glob("/tmp/" + cmd.Message_Params.Params.S3_info.Destination.S3_file.Path + "/" + cmd.Message_Params.Params.S3_info.Destination.S3_file.Ts_part + "/*")
	if err != nil {
		log.Error().Msgf("Error on list directory: %s\n", err.Error())
		cmd.Message_Params.Status.Set("FAILURE")
		cmd.Output <- cmd.Message_Params
		return
	}
	for _, filepath := range files {
		...
		putinfo, err := minioClient.FPutObject(cmd.Message_Params.Params.S3_info.Destination.S3_file.Bucket, cmd.Message_Params.Params.S3_info.Destination.S3_file.Path+"/"+filenamelist[len(filenamelist)-1], filepath, minio.PutObjectOptions{})
...
func main() {
	log.Info().Msgf("Go-FFmpeg-PVI has started")
