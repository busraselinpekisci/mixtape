...
func (cmd *Worker) Run() {
	...
	for _, c := range cmd.Message_Params.Params.Commands {
		...
		if err != nil {
			log.Error().Msgf("Command: %v execution error: %s\n  Error Message: %s", c.Cmd, err.Error(), string(out))
			cmd.Message_Params.Status.Set("FAILURE")
			cmd.Output <- cmd.Message_Params
			return
		}
	...
	files, err := filepath.Glob("/tmp/" + cmd.Message_Params.Params.S3_info.Destination.S3_file.Path + "/*")
	if err != nil {
...
func main() {
	...
	log.Info().Msgf("Go-FFmpeg-Hls has started")
